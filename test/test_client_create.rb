...
class TestClientCreate < ParseTestCase
  ...
  def test_simple_save
    VCR.use_cassette('test_simple_save', :record => :new_episodes) do
      test_save = Parse::Object.new("TestSave", data = nil, client = @client)
      test_save["foo"] = "bar"
      test_save.save
  ...
  def test_update
    VCR.use_cassette('test_update', :record => :new_episodes) do
      foo = Parse::Object.new("TestSave", data = nil, client = @client)
      foo["age"] = 20
      foo.save
  ...
  def test_server_update
    VCR.use_cassette('test_server_update', :record => :new_episodes) do
      foo = Parse::Object.new("TestSave", data = nil, client = @client).save
      foo["name"] = 'john'
      foo.save

      bar = Parse.get("TestSave", foo.id, client = @client) # pull it from the server
      assert_equal bar["name"], 'john'
      bar["name"] = 'dave'
      bar.save

      bat = Parse.get("TestSave", foo.id, client = client)
  ...
  def test_destroy
    VCR.use_cassette('test_destroy', :record => :new_episodes) do
      d = Parse::Object.new("toBeDeleted", data = nil, client = @client)
      d["foo"] = "bar"
      d.save
      d.parse_delete
  ...
  def test_get_missing
    VCR.use_cassette('test_get_missing', :record => :new_episodes) do
      e = assert_raise(Parse::ParseProtocolError) do
        Parse.get("SomeClass", "someIdThatDoesNotExist", client = @client)
      end
